...
class DeclarativeMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        for option in [option for option in dir(options) if not option.startswith("_")]:
            setattr(meta, option, getattr(options, option))
...
class ModelDeclarativeMetaclass(DeclarativeMetaclass):
    def __new__(cls, name, bases, attrs):
        ...
        if opts.model:
            ...
            for p in get_properties(opts.model):
                pname, ptitle = p

                if opts.exclude and pname in opts.exclude:
                    continue
                if opts.fields and pname in opts.fields:
                    name = pname
                    if opts.map_properties is not None and opts.map_properties.get(
                        pname
                    ):
                        name = opts.map_properties.get(pname)
                    # if p in new_class.
                    field_list.append(
                        (
                            pname,
                    ...
                    # declared_fields[p] = Field(
                    #     attribute=p, readonly=True, default="",)
            ...
            if opts.fields is not None:
                ...
                for field_name in opts.fields:
                    ...
                    for i, attr in enumerate(attrs):
                        ...
                        # check if it 's in properties also! by milia

                        if [
                            i
                            for i, v in enumerate(get_properties(model))
                            if v[0] == attr
                        ]:
                            f = attr
                        else:
                            try:
                                f = model._meta.get_field(
                                    attr,
                                )
...
class ModelResource(Resource, metaclass=ModelDeclarativeMetaclass):
    ...
    @classmethod
    def field_from_django_property(
        cls,
        field_name,
    ):
        """
        ...
        FieldWidget = cls.widget_from_django_field(field_name)
        widget_kwargs = cls.widget_kwargs_for_field(field_name)
        field = cls.DEFAULT_RESOURCE_FIELD(
            attribute=field_name,
            column_name=field_name,
...
class StyledResource(ModelResource):
    def after_export(self, queryset, dataset, **kwargs):
        # Create a new workbook
        workbook = Workbook()
        worksheet = workbook.active
        # Add a title
        # Populate the worksheet with dataset data
        headers = dataset.headers
        worksheet.append(headers)  # Add headers
        # if kwargs.get("title", None) is not None:
        #     self.add_table_title(worksheet, "sdsdd")
        for row in dataset.dict:
            # Process only numeric values (float/int) in non-standard formats
            processed_row = [
                float(str(value).replace("\xa0", "").replace(",", "."))
        ...
        # for row in dataset.dict:
        #     worksheet.append(list(row.values()))  # Add data rows
        ...
        self.style_header_row(worksheet)
        self.apply_background(worksheet)
        # Set filters for headers
        self.set_auto_filter(worksheet, len(headers))
        ...
        # Save the workbook to a BytesIO stream if needed for response
        return workbook

    def set_auto_filter(self, worksheet, column_count):
    ...
    def apply_borders(self, worksheet):
    ...
    def apply_background(self, worksheet):
        """Apply borders to all cells in the worksheet."""
        currency_format = NamedStyle(name="currency")
        currency_format.number_format = "#,##0.00"
        for row in worksheet.iter_rows():
