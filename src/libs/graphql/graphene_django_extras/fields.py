...
from libs.graphql.graphene_django_extras.filters.filter import get_filterset_class
...
class DjangoFilterListField(Field):
    ...
    def list_resolver(manager, filterset_class, filtering_args, root, info, **kwargs):
        ...
        if qs is None:
            try:
                qs = queryset_factory(
                    manager, info.field_nodes, info.fragments, **kwargs
                )
                qs = filterset_class(
...
class DjangoFilterPaginateListField(Field):
    ...
    def list_resolver(
        ...
        qs = filterset_class(
            data={"contacts__in": "[1,2]"}, queryset=qs, request=info.context
        ).qs
