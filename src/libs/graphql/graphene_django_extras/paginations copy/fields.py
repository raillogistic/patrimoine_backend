...
__all__ = ("LimitOffsetPaginationField",
           "PagePaginationField", "CursorPaginationField")
...
class LimitOffsetPaginationField(AbstractPaginationField):
    def __init__(
        ...
        **kwargs,
        ...
        super(LimitOffsetPaginationField, self).__init__(
            List(_type), *args, **kwargs)

    def list_resolver(self, manager, root, info, **kwargs):
        print("mldkmsldksmdlksmldksmldkml")
...
class PagePaginationField(AbstractPaginationField):
    ...
    def list_resolver(self, manager, root, info, **kwargs):
        ...
        offset = (
            int(count - fabs(page_size * page)
                ) if page < 0 else page_size * (page - 1)
        )
...
class CursorPaginationField(AbstractPaginationField):
    def __init__(
        ...
        super(CursorPaginationField, self).__init__(
            List(_type), *args, **kwargs)

    def list_resolver(self, manager, root, info, **kwargs):
        print("xqskdlkqsdmlqskdmk")
