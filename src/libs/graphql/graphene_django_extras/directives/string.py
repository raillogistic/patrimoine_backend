...
class Base64GraphQLDirective(BaseExtraGraphQLDirective):
    ...
    def resolve(value, directive, root, info, **kwargs):
        ...
        op_argument = [
            arg for arg in directive.arguments if arg.name.value == "op"]
...
class NumberGraphQLDirective(BaseExtraGraphQLDirective):
    ...
    def resolve(value, directive, root, info, **kwargs):
        as_argument = [
            arg for arg in directive.arguments if arg.name.value == "as"][0]
...
class CenterGraphQLDirective(BaseExtraGraphQLDirective):
    ...
    def resolve(value, directive, root, info, **kwargs):
        ...
        width_argument = (
            width_argument[0].value.value if len(
                width_argument) > 0 else len(value)
        )
        ...
        fillchar_argument = (
            fillchar_argument[0].value.value if len(
                fillchar_argument) > 0 else " "
        )
...
class ReplaceGraphQLDirective(BaseExtraGraphQLDirective):
    ...
    def resolve(value, directive, root, info, **kwargs):
        old_argument = [
            arg for arg in directive.arguments if arg.name.value == "old"]
        old_argument = old_argument[0].value.value if len(
            old_argument) > 0 else None

        new_argument = [
            arg for arg in directive.arguments if arg.name.value == "new"]
        new_argument = new_argument[0].value.value if len(
            new_argument) > 0 else None
