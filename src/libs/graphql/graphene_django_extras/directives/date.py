...
def _format_time_ago(dt, now=None, full=False, ago_in=False, two_days=False):
    ...
    if not isinstance(dt, timedelta):
        ...
        flag, result = _format_relativedelta(
            result, full, two_days, original_dt)
        if ago_in and flag is not None:
            result = "in {}".format(
                result) if flag else "{} ago".format(result)
        return result
...
class DateGraphQLDirective(BaseExtraGraphQLDirective):
    ...
    def resolve(value, directive, root, info, **kwargs):
        ...
        format_argument = format_argument[0] if len(
            format_argument) > 0 else None
